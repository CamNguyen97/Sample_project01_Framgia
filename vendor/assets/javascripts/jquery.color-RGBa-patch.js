(function(jQuery) {
  jQuery.each(['backgroundColor', 'borderBottomColor', 'borderLeftColor',
    ...
    jQuery.fx.step[attr] = function(fx) {
      if (fx.colorFunction == undefined || fx.state == 0) {
        ...
        fx.colorFunction = (fx.start[3] == 1 && fx.end[3] == 1 ?
          calcRGB : calcRGBa);
      }
  ...
  var calcRGB = function() {
    return 'rgb(' +
      Math.max(Math.min(parseInt((this.pos * (this.end[0] - this.start[0])) +
        this.start[0]), 255), 0) + ',' +
      Math.max(Math.min(parseInt((this.pos * (this.end[1] - this.start[1])) +
        this.start[1]), 255), 0) + ',' +
      Math.max(Math.min(parseInt((this.pos * (this.end[2] - this.start[2])) +
        this.start[2]), 255), 0) + ')';
  };
  var calcRGBa = function() {
    return 'rgba(' +
      ...
      Math.max(Math.min(parseInt((this.pos * (this.end[2] - this.start[2])) +
        this.start[2]), 255), 0) + ',' +
      Math.max(Math.min(parseFloat((this.pos * (this.end[3] - this.start[3])) +
        this.start[3]), 1), 0) + ')';
  };
  ...
  function getRGB(color) {
    ...
    if (result =
      ...
      return [parseFloat(result[1]) * 2.55, parseFloat(result[2]) * 2.55,
        parseFloat(result[3]) * 2.55, parseFloat(result[4] || 1)
      ];
    if (result = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/.exec(
        color))
      return [parseInt(result[1], 16), parseInt(result[2], 16), parseInt(
        result[3], 16)];
    if (result = /#([a-fA-F0-9])([a-fA-F0-9])([a-fA-F0-9])/.exec(color))
      return [parseInt(result[1] + result[1], 16), parseInt(result[2] +
